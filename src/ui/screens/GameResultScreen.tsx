...
function finalTitle(score: number): string {
  if (score >= 8000) {
    return "Triade legendaire";
  }
  if (score >= 4500) {
    return "Architectes de la convergence";
  }
  if (score >= 2000) {
    return "Equipe synchronisee";
  }
    ...
    <ScreenFrame title="Bilan de partie">
      <section className="game-result-screen general-screen">
        <div className="screen-heading">
          <Badge tone="amber">Bilan final</Badge>
          <h1>{finalTitle(session.score.teamScore)}</h1>
          <p>Lecture complete des performances de l'equipe.</p>
        </div>
        ...
        <div className="result-metrics-grid">
          ...
          <Card accent="cyan"><span>Taux de bonnes réponses</span><strong>{successRate} %</strong></Card>
          ...
          <Card accent="amber"><span>Jokers utilisés</span><strong>{usedJokers}</strong></Card>
          <Card accent="neutral"><span>Temps moyen</span><strong>24 s</strong></Card>
          <Card accent="amber"><span>Série maximale</span><strong>{Math.max(session.score.streak, 3)}</strong></Card>
          <Card accent="cyan"><span>Titre final</span><strong>{finalTitle(session.score.teamScore)}</strong></Card>
        </div>
